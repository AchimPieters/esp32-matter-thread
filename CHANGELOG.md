...
### Changed
- Updated LED example/component manifests for ESP-IDF v5.4.x (`idf >= 5.4`) and pinned `espressif/esp_matter` to `1.4.2~1`.
- Added local `components/json` compatibility shim (backed by `cjson`) for ESP-IDF v6.0 compatibility with legacy `esp_matter` dependency naming.
- Added managed dependency `espressif/mqtt` in manifests for IDF v6.0 component resolution.
