 ...
 * @param[in]   ctx   esp_decrypt_handle_t handle
 *
 * @note This API cleans the decrypt handle and return ESP_FAIL if the complete data has not been decrypted. Verify if
 * complete data has been decrypted using API `esp_encrypted_img_is_complete_data_received` to prevent an early call to
 * this API.
 *
 * @return
 *    - ESP_FAIL                 On failure
 ...
 *
 * @note This API checks if complete data has been supplied to `esp_encrypted_img_decrypt_data`. This can be used to
 * prevent an early call to `esp_encrypted_img_decrypt_end` which cleans up the decrypt handle. If this API returns
 * true, then call `esp_encrypted_img_decrypt_end`. If this API returns false, and there is some other error (like
 * network error) due to which decryption process should be terminated, call `esp_encrypted_img_decrypt_abort` to clean
 * up the handle.
 *
 * @param[in]  ctx   esp_decrypt_handle_t handle
 *
